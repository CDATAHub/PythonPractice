...
if __name__ == "__main__":
    ...
    # 函数式编程，map()、filter()和reduce()，通常结合匿名函数lambda⼀起使⽤
    # 将列表元素加倍
    ...
    # 计算列表之和 (代替 lambda x, y: x + y)
    res = reduce(operator.add, [1, 2, 3, 4])  # 10

    # 计算阶乘 (代替 lambda x, y: x * y)
    res = reduce(operator.mul, [1, 2, 3, 4])  # 24

    # 其他：sub(-), truediv(/), floordiv(//), mod(%), pow(**)
